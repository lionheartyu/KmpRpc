...
void RpcProvider::Run()
{
    ...
    uint16_t port = atoi(MprpcApplication::GetInstance().GetConfig().Load("rpcserverport").c_str()); // c_str is  string->char*
    muduo::net::InetAddress address(ip, port);

    // 创建TCPServer对象
    muduo::net::TcpServer server(&m_eventloop, address, "RpcProvider");

    // 绑定连接回调和消息读写回调回调  分离了网络代码和业务代码
    ...
    //session timeout  30s   zkcient网络I/O线程  
    ZkClient zkCli;
    zkCli.Start();
    // servie_name为永久性节点 method_name为临时节点
...
service_name method_name  args  定义proto的message类型 , 进行数据头的序列胡化 和 反序列化
                                service_name method_name args_size
16UserServiceLoginzhangsan123

header_size(4个字节) + header_str + args_str
            =>按二进制存
std::string  insert 和 copy方法
*/
                            ...
                            muduo::Timestamp)
{
    ...
    // 根据headedr_size读取数据头的原始字符流,反序列化数据,得到rpc请求的详细信息
    ...
    {
        // 数据头反序列化失败
        // std::cout << "rpc_hearder_str:" << rpc_header_str << "parse error!" << std::endl;
        LOG_ERROR("rpc_hearder_str:%s", rpc_header_str.c_str());
    ...
    if (mit == it->second.m_methodMap.end())
    {
        ...
        LOG_ERROR("%s%sis not exist!", service_name.c_str(), method_name.c_str());
        LOG_ERROR("%s:%s:%d", __FILE__, __FUNCTION__, __LINE__);
        return;
    }
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response)
{
    ...
    conn->shutdown(); // 模拟http的短链接服务,由rpcprovidr主动断开连接
}
