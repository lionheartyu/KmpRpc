                              ...
                              google::protobuf::Closure *done)
{
    ...
    // send_rpc_str.insert(4, rpc_header_str);
    // send_rpc_str.insert(4 + header_size, args_str);
    ...
    // 连接rpc服务节点
    if (-1 == connect(clintfd, (struct sockaddr *)&ServerAddr, sizeof(ServerAddr)))
    {
        char errtxt[512] = {0};
    ...
    int recv_size = 0;
    if (-1 == (recv_size = recv(clintfd, recv_buf, sizeof(recv_buf), 0)))
    {
        char errtxt[512] = {0};
        sprintf(errtxt, "recv errno! errno:%d", errno);
        controller->SetFailed(errtxt);
        close(clintfd);
        return;
    }
    ...
    {
        char errtxt[512] = {0};
        sprintf(errtxt, "parse error! recv_buf:%d", errno);
        controller->SetFailed(errtxt);
        close(clintfd);
        return;
    }
